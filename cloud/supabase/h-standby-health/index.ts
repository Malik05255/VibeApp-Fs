...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const observationAgeSeconds = lastReplicationAt == null
      ? null
      : Math.max(0, (Date.now() - lastReplicationAt) / 1000);
    const replicationFresh = replicationContinuous &&
      replicationLagSeconds != null && replicationLagSeconds <= MAX_REPLICATION_LAG_SECONDS &&
      observationAgeSeconds != null && observationAgeSeconds <= MAX_REPLICATION_OBSERVATION_AGE_SECONDS;
    ...
    return reply({
      ...
      runtimeRole: runtimeRoleOk ? "standby" : "unknown",
      hIdentity: hIdentityOk ? "H" : "unknown",
      runtimeReady,
      restoreVerified,
      replicationMode: replicationMode || "unknown",
      replicationLagSeconds,
      replicationObservationAgeSeconds: observationAgeSeconds,
      maxReplicationLagSeconds: MAX_REPLICATION_LAG_SECONDS,
      maxReplicationObservationAgeSeconds: MAX_REPLICATION_OBSERVATION_AGE_SECONDS,
      standbyReady,
      storageOnlyBackupAccepted: false,
...
async function loadStandbyState(db: DbClient): Promise<StateRow[]> {
  const { data, error } = await db.from("h_runtime_state")
    ...
    .in("key", ["standby_runtime", "standby_replication"]);
  if (error) throw error;
