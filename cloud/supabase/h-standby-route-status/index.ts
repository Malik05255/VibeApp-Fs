...
Deno.serve(async (req: Request) => {
  ...
  try {
    // This endpoint is a narrow control-plane status probe. It may authenticate an owner on
    // a passive standby, but it never performs user execution and independently requires a
    // complete request-only promotion attestation before returning activeReady=true.
    ...
    const [runtime, execution, promotion, replication] = await Promise.all([
    ...
    const sourceDigest = text(promotion.source_digest)?.toLowerCase() ?? null;
    const replicationDigest = text(replication.last_digest)?.toLowerCase() ?? null;
    ...
    const restoreVerified =
      text(replication.mode) === "continuous" &&
      text(replication.protocol) === REPLICATION_PROTOCOL &&
      replication.exact_mirror === true &&
      Boolean(replicationDigest && DIGEST_PATTERN.test(replicationDigest));
    ...
    const promotionAttested =
      ...
      restoreVerified &&
      sourceDigest === replicationDigest &&
    ...
    return reply({
      ...
      autonomousOutboundActive,
      restoreVerified,
      replicationProtocol: text(replication.protocol),
      checkedAt: new Date().toISOString(),
      credentialsExposed: false,
      runtimeSecretExposed: false,
      routingIdentityExposed: false,
    });
  } catch (error) {
