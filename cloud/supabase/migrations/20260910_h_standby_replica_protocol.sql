...
begin
  ...
  -- Keep this first protocol deliberately bounded. A future paginated protocol must be
  -- introduced instead of silently truncating operational state.
  ...
  -- Delete dependent tables first. The standby is not writable by users while this mode
  -- is enabled, so a full H-table mirror is safer than merge semantics for failover.
