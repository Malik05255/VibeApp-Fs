...
- The live Peach polling path is default-deny. It resolves owners from `h_runtime_owner_identities` and explicitly enrolled friends from `h_runtime_friend_identities`; both tables store only `poll_secret`-keyed HMAC fingerprints, never raw WhatsApp numbers. Unknown or unenrolled senders are blocked before their message body/raw payload is persisted or sent to H chat/AI.
- A valid one-time owner-pairing command is the only pre-enrollment exception. Pairing input is redacted before persistence and consumed from an HMAC fingerprint.
- `h-owner-config` is an internal runtime-secret-protected endpoint for explicit owner and friend enrollment/removal/status. Friend operations are `enroll_friend` and `remove_friend`. The endpoint never returns raw WhatsApp numbers or stored fingerprints.
- Owner contact save and third-party WhatsApp send/schedule commands execute in the same Supabase H runtime and `h_runtime_contacts` store. Friends never receive that capability, even if their natural-language request asks for external delivery.
