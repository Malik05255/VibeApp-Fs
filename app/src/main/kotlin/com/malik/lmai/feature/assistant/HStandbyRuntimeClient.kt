...
class HStandbyRuntimeClient @Inject constructor(
    ...
    suspend fun createSetupLink(): HCloudLinkResponse = withContext(Dispatchers.IO) {
        ...
        executePost(refreshed, payload).also(::rememberTargetEndpoint)
    }

    private fun rememberTargetEndpoint(response: HCloudLinkResponse) {
        if (!response.ok) return
        val endpoint = (response.body["targetEndpoint"] as? JsonPrimitive)?.content?.trim().orEmpty()
        if (endpoint.isNotEmpty()) runtimeRouteStore.rememberStandbyEndpoint(endpoint)
    }
